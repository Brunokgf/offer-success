...
export async function handler(event) {
  ...
  if (!secretKey) return json(500, { error: "MEDUSAPAY_SECRET_KEY não configurado na Netlify." });

  let data;
  try {
    data = PixInput.parse(JSON.parse(event.body || "{}"));
  } catch {
  ...
  const body = {
    paymentMethod: "pix",
    ip: event.headers["x-forwarded-for"]?.split(",")[0]?.trim() || "127.0.0.1",
    pix: { expiresInDays: 1 },
  ...
  try {
    ...
    if (!res.ok || !payload) {
      ...
      if (res.status === 424 || /ENOTFOUND|Fnt/i.test(raw)) {
        return json(502, {
          error:
            "MedusaPay está com instabilidade no PIX. Tente em alguns minutos ou finalize pelo WhatsApp.",
        });
      }
